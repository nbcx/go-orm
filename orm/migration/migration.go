...
func Upgrade(lasttime int64) error {
	...
	for _, v := range sm {
		if _, ok := migs[v.name]; !ok {
			...
			log.Info("end upgrade:", v.name)
			i++
		}
	}
	log.Info("total success upgrade:", i, " migration")
	time.Sleep(2 * time.Second)
	return nil
}
...
func Rollback(name string) error {
	...
	log.Error("not exist the migrationMap name:" + name)
	time.Sleep(2 * time.Second)
...
func Reset() error {
	...
	for j := len(sm) - 1; j >= 0; j-- {
		...
		if isRollBack(v.name) {
			log.Info("skip the", v.name)
			time.Sleep(1 * time.Second)
			continue
		}
		log.Info("start reset:", v.name)
		v.m.Reset()
		v.m.Down()
		...
		i++
		log.Info("end reset:", v.name)
	}
	log.Info("total success reset:", i, " migration")
	time.Sleep(2 * time.Second)
	return nil
}
...
func isRollBack(name string) bool {
	...
	if err != nil {
		log.Info("get name has error", err)
		return false
	}
	if num <= 0 {
		return false
