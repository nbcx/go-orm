...
import (
	...
	iutils "github.com/nbcx/go-orm/orm/internal/utils"
	"github.com/nbcx/log"
...
func (o *ormBase) QueryM2MWithCtx(_ context.Context, md interface{}, name string) QueryM2Mer {
	log.Warn("QueryM2MWithCtx is DEPRECATED. Use methods with `WithCtx` suffix on QueryM2M as replacement please.")
...
func (o *ormBase) QueryTableWithCtx(_ context.Context, ptrStructOrTableName interface{}) (qs QuerySeter) {
	log.Warn("QueryTableWithCtx is DEPRECATED. Use methods with `WithCtx` suffix on QuerySeter as replacement please.")
...
func doTxTemplate(ctx context.Context, o TxBeginner, opts *sql.TxOptions,
	...
	defer func() {
		if panicked || err != nil {
			e := _txOrm.Rollback()
			if e != nil {
				log.Error("rollback transaction failed: %v,%v", e, panicked)
			}
		} else {
			e := _txOrm.Commit()
			if e != nil {
				log.Error("commit transaction failed: %v,%v", e, panicked)
			}
		}
	}()
	taskTxOrm := _txOrm
